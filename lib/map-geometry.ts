...
export function layoutBlockToLatLngs(
  block: {
    x: number;
    y: number;
    width: number;
  ...
  if (block.type === "polygon" && Array.isArray(block.polygonPoints) && block.polygonPoints.length >= 3) {
    return block.polygonPoints.map((point) => percentToLatLng(point.x, point.y, calibration));
  }

  return blockPolygonToLatLngs(block, calibration, offsetX);
}
